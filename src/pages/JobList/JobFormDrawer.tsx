...
const JobFormDrawer: React.FC<JobFormDrawerProps> = ({
  ...
  const provinceCityMap: Record<string, string[]> = {
    ...
    江苏省: ['南京市', '苏州市', '无锡市', '常州市', '南通市', '扬州市', '徐州市', '连云港市', '淮安市', '盐城市', '镇江市', '泰州市', '宿迁市'],
  ...
  useEffect(() => {
    if (visible) {
      if (initialData) {
        ...
        let city = locationParts.slice(1).join('-') || '';
        // 直辖市处理：如果无市数据，默认市等于省
        if (!city && ['北京市', '上海市', '天津市', '重庆市'].includes(province)) {
          city = province;
        }
  ...
  const handleFinish = (values: any) => {
    const job: JobItem = {
      ...
      workLocation: values.province === values.city ? values.province : `${values.province}-${values.city}`,
                    ...
                    >
                      {Object.keys(provinceCityMap).map((province) => (
                        <Option key={province} value={province}>
                          {['北京市', '上海市', '天津市', '重庆市'].includes(province) ? province.replace(/市$/, '') : province}
                        </Option>
                      ))}
                    </Select>
                  </Form.Item>
